...
class ImageSourceTest {
    ...
    void createImageSource(String url) {
    ...
    void createImageSourceWithoutHttpAndHttps(String url) {
    ...
    void createImageSourceWithoutProtocolAndWww(String url) {
        assertThatCode(() -> new ImageSource(url)).doesNotThrowAnyException();
    }
    ...
    void createImageSourceWithManySteps(String url) {
        assertThatCode(() -> new ImageSource(url)).doesNotThrowAnyException();
    }
    ...
    void createImageSourceWithOneStep(String url) {
        assertThatThrownBy(() -> new ImageSource(url)).isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    @DisplayName("-_가 포함된 이미지 주소를 생성할 수 있다.")
    void createImageSourceWithSpecialChar() {
